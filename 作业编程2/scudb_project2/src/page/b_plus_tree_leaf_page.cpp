...
int B_PLUS_TREE_LEAF_PAGE_TYPE::KeyIndex(
  ...
  assert(GetSize() >= 0);
  int start = 0, tempend = GetSize() - 1;
  while (start <= tempend ) { //find the last key in array <= input
    int zhongjian = (tempend  - start) / 2 + start;
    if (comparator(array[zhongjian].first,key) >= 0) tempend  = zhongjian - 1;
    else start = zhongjian + 1;
  }
  return tempend  + 1;
}
...
int B_PLUS_TREE_LEAF_PAGE_TYPE::Insert(const KeyType &key,
  ...
  int suoyin = KeyIndex(key,comparator); //first larger than key
  assert(suoyin >= 0);
  IncreaseSize(1);
  int currentSize = GetSize();
  for (int temp = currentSize - 1; temp > suoyin; temp--) {
    array[temp].first = array[temp - 1].first;
    array[temp].second = array[temp - 1].second;
  }
  array[suoyin].first = key;
  array[suoyin].second = value;
  return currentSize;
}
...
void B_PLUS_TREE_LEAF_PAGE_TYPE::MoveHalfTo(
  ...
  int totalnumber = GetMaxSize() + 1;
  assert(GetSize() == totalnumber);
  //copy last half
  int copysuoyin = (totalnumber)/2;//7 is 4,5,6,7; 8 is 4,5,6,7,8
  for (int temp = copysuoyin; temp < totalnumber; temp++) {
    recipient->array[temp - copysuoyin].first = array[temp].first;
    recipient->array[temp - copysuoyin].second = array[temp].second;
  }
  //set pointer
  ...
  SetSize(copysuoyin);
  recipient->SetSize(totalnumber - copysuoyin);
...
bool B_PLUS_TREE_LEAF_PAGE_TYPE::Lookup(const KeyType &key, ValueType &value,
  ...
  int suoyin = KeyIndex(key,comparator);
  if (suoyin < GetSize() && comparator(array[suoyin].first, key) == 0) {
    value = array[suoyin].second;
    return true;
  }
  return false;
}
...
void B_PLUS_TREE_LEAF_PAGE_TYPE::MoveAllTo(BPlusTreeLeafPage *recipient,
  ...
  //copy last half
  int startSuoYin = recipient->GetSize();//7 is 4,5,6,7; 8 is 4,5,6,7,8
  for (int temp = 0; temp < GetSize(); temp++) {
    recipient->array[startSuoYin + temp].first = array[temp].first;
    recipient->array[startSuoYin + temp].second = array[temp].second;
  }
  //set pointer
  recipient->SetNextPageId(GetNextPageId());
  //set size, is odd, bigger is last part
...
std::string B_PLUS_TREE_LEAF_PAGE_TYPE::ToString(bool verbose) const {
  ...
  while (rukou < tempEnd) {
    ...
    stream << std::dec << array[rukou].first;
    if (verbose) {
      stream << "(" << array[rukou].second << ")";
    }
    ++rukou;
  }
  return stream.str();
